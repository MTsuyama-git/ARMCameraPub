...
uint8_t packet_parse(packet_t* p)
{
  ...
  while (1) {
	...
	p->data[i] = ring_buffer_index_with_offset(&rx, i, rx.save_ptr);
      ...
      uint16_t crc16_calc = calc_crc16((uint32_t*)(&p->data[0]), len + 8);
      if (crc16 == crc16_calc) {
	p->type = type;
...
uint32_t packet_serialize(packet_t const*p, uint8_t* dest, uint32_t size) {
  ...
  uint16_t crc16 = calc_crc16((uint8_t*)dest, p->len+8);
  dest[p->len+8]  = (uint8_t)(crc16         & 0xFF);
  dest[p->len+9]  = (uint8_t)((crc16 >> 8)  & 0xFF);
  return p->len+10;
}
