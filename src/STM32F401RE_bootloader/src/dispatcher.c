...
void send_upstream_packet(packet_t const* p)
{
  uint8_t size = packet_serialize(p, tx_buffer, 64);
  uart_send_downstream(tx_buffer, size);
  // handlers.send_upstream(tx_buffer, size);
}
...
static uint32_t erase_firm()
{
    ...
    erase_info.Sector = FLASH_SECTOR_2;
    erase_info.NbSectors     = 4;
